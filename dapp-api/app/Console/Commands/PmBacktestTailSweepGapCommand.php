...
class PmBacktestTailSweepGapCommand extends Command
{
    ...
    private function printMode1(array $result): void
    {
        ...
        if ($result['details'] !== []) {
            $this->table(
                ['ID', '开始时间', '上一轮价格', '当前轮价格', '当前预测', '实际方向', '价差绝对值', '投注金额', '结果', '总投资金额', '累计净盈利', '净盈利占比', '连亏次数', '资金占用', '重置次数', '下一轮预测', '下一次命中ID', '下一轮投注'],
                $result['details']
            );
        }
    }
    ...
    {
        $this->line('涨跌口径：结束价格 > 开始价格 记为涨，否则记为跌');
        if ($result['message']) {
